...
|---|---|---|---|
| S01 | ZIP completo no destino | CORRIGIDO_JVM | `ScribeBackupManager.exportBackup` chama finish/flush; `s01_exportedZipHasValidCentralDirectory...` passa abrindo ZipFile. `ExpansionsViewModel.createBackup` sincroniza o descritor aberto. Não equivale à integridade transacional do conteúdo. |
| S02 | Backup cobre dados reais e contagens corretas | PARCIAL | Diretórios canônicos adicionados; teste independente reprova omissão de `personal_styles.json`. Dados Guided em `filesDir/strokes`/manifesto também ficam fora. Glifos contam variantes; histórico conta arquivo. Unificar caminhos/inventário (V4-02). |
| S03 | Restore sem alteração parcial, com recuperação | ABERTO | Staging/rollback existem; teste após primeira cópia deixa `notebooks/` novo. Commit não atômico e sem recuperação após interrupção. Provar falha por etapa/inventário (V4-01). |
| S04 | Manifesto inválido nunca autoriza sobrescrita | ABERTO | Não-JSON e versão 999 aceitos; import substitui bytes e retorna sucesso. Rejeição estrita antes do commit exigida (V4-01). |
...
| S07 | Professor usa novas tentativas reais, não seeds | PARCIAL | Construção do repositório de tentativas não chama seeds; Guided salva avaliação real. Teacher aberto antes da escrita lê cache antigo, reproduzido. Dados sintéticos preexistentes não são isolados (V4-03). |
| S08 | Ausência não vira nota/maturidade inventada | PARCIAL | Diagnóstico vazio corrigido e testado; gerador ainda afirma maturidade superior para weakness=null. Medição ausente pode virar alvo no registro de tentativa (V4-04/06). |
| S09 | Diagnóstico mede pauta/estilo e respeita amostragem | PARCIAL | Status insuficiente e ângulo ponderado adicionados. Contenção ainda é score geral; reta densa perde medição; alvo 52° não acompanha cada estilo. Provas independentes falham (V4-04). |
...
| S11 | Reanálise fora da main, ordenada e recuperável | ABERTO | `TeacherViewModel.reanalyzeAllData` mantém launch padrão, IO síncrono/leitura e análise na main, sem finally. Três gravações independentes/tmp fixo. Implementação alegada não localizada (V4-04). |
...
| S14 | Persistir referência e restaurar View | ABERTO | `loadBaselineSignature` vazio; saveAsBaseline somente StateFlow; AndroidView não usa setStrokes. Requer vetor em disco, reabertura/recriação (V4-05). |
| S15 | Export enquadrado e SVG entregue como arquivo | PARCIAL | SVG físico implementado; coordenadas negativas permanecem fora do viewport. Cores/larguras alteradas; destinos privados. PNG/pixels pendentes (V4-05). |
| S16 | Repetibilidade não afirma equivalência que não mede | PARCIAL | Máximos negativos corrigidos; diagonais opostas ainda recebem 100 com dinâmica muscular idêntica. Prova independente falha. Limitar linguagem ou implementar comparação requerida (V4-05). |
...
| S24 | Serialização estrita, corrupção preservada/erro observável | ABERTO | Regex/defaults em Backup/Teacher/PersonalStyle; teste de aspas/barras/newline falha; caches/fallbacks silenciosos. Necessário parser/esquema e round-trip integral (V4-01/07). |
