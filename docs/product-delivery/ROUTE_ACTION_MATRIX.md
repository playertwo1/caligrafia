...
| **F6-RT-01** | Preferências | mão/lado/entrada | editor | opções F6 | `ExpansionsViewModel` | `ScribePreferencesStore` | `PhaseF6AcceptanceTest` | SOFTWARE_VALIDADO; FÍSICO PENDENTE |
| **F6-RT-02** | Preferências | texto/contraste/animação | efeito global/editor | opções F6 | runtime/render/editor | SharedPreferences | compilação local | SOFTWARE_IMPLEMENTADO |
| **F6-RT-03** | Preferências | pausa/intervalo/vibração | alerta por escrita ativa | duração ativa | canvas → ViewModel → reminder | SharedPreferences | `PhaseF6AcceptanceTest` | SOFTWARE_VALIDADO; FÍSICO PENDENTE |
| **F6-RT-04** | S Pen e Watch | consultar conexão | status/fallback | permissão + GATT | `WatchCompanionAdapter` | Android Bluetooth | revisão estática | PENDENTE_DISPOSITIVO |
| **F6-RT-05** | Laboratório | gravar/parar/salvar/reabrir/replay | amostra isolada | raw strokes | `StylusLabViewModel` | diretórios exclusivos | compilação local | SOFTWARE_IMPLEMENTADO; E2E PENDENTE |
| **F6-RT-06** | Backup | restaurar preferências | snapshot completo/legado | chaves permitidas | `ScribePreferencesStore` | `preferences_snapshot.txt` | `ScribePreferencesBackupTest` | SOFTWARE_VALIDADO |
