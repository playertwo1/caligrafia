...
| F6.05 | Escala de texto | Multiplicar fontScale do sistema; controles essenciais permanecem acessíveis | `text_scale` | `MainActivity` fornece `LocalDensity` com `system fontScale × multiplier` | **SOFTWARE_IMPLEMENTADO; controle/preview UI e teste visual pendentes** |
| F6.06 | Contraste das guias | Alterar somente alpha do Paint das pautas | `guide_contrast` + compatibilidade `is_high_contrast` | `GuidelineRenderer` + bridge reativo de chaves legadas | **SOFTWARE_IMPLEMENTADO; preview UI pendente** |
...
| F6.08 | Curva Linear/Suave/Firme | Alterar apenas largura/render derivado | `pressure_curve` | `SmoothedReferenceRenderer`, `NotebookCanvasView`, `StylusLabViewModel`, bridge reativo legado | **SOFTWARE_IMPLEMENTADO; seletor antigo ainda oferece SIGMOID e precisa ser limitado na UI** |
...
| F6.10 | Lembrete de pausa | Contar somente escrita ativa em foreground; pausa/fundo não acumulam | `break_reminder_enabled`, `break_interval_minutes` | UI já possui toggle/slider; `WatchCompanionAdapter` persiste; `ActiveBreakReminder` existe | **UI+NÚCLEO_PARCIAIS; ligação do relógio à sessão real pendente** |
| F6.11 | Vibração | Habilitar/desabilitar; retornar false se aparelho/permissão indisponível | `vibration_enabled` | UI já possui botão Testar Pulso Háptico; bridge trata capability/erro | **TESTE_HAPTICO_EXISTE; toggle de vibração e teste físico pendentes** |
| F6.12 | Watch conectado | Nunca alegar conexão sem sinal real; fallback permanece no telefone | probe de GATT + `BLUETOOTH_CONNECT` | `WatchCompanionAdapter.isWatchConnected()` | **PARCIAL — probe real implementado; permissão runtime, status UI e validação Watch pendentes** |
...
| F6.17 | Reinício | Preferências e raw do laboratório sobrevivem reinício | SharedPreferences + `.scribe` de laboratório | store + save/load existentes; gap F5↔F6 documentado | **PENDENTE_TESTE_EXECUTADO; backup completo das novas preferências ainda bloqueia** |
...
**NÃO FECHADO.** O núcleo de persistência e várias políticas de render/entrada foram implementados, porém há itens de UI, integração de sessão, round-trip de preferências e hardware físico pendentes. Esta matriz é a fonte de passagem de bastão para concluir o gate sem criar falso positivo.
