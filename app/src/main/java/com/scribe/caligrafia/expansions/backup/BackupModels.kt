package com.scribe.caligrafia.expansions.backup

/**
 * Manifesto descritivo de um pacote de backup Scribe (.scribepack).
 * Campos novos possuem defaults para preservar leitura de pacotes 1.0 anteriores.
 */
data class BackupManifest(
    val formatVersion: String = "1.0",
    val appVersion: String = "0.8.5",
    val appVersionCode: Int = 15,
    val createdAtMs: Long = System.currentTimeMillis(),
    val deviceInfo: String = "Android",
    val notebookCount: Int = 0,
    val pageCount: Int = 0,
    val personalGlyphCount: Int = 0,
    val lessonHistoryCount: Int = 0,
    val spacedRepetitionCount: Int = 0,
    val practiceAttemptCount: Int = 0,
    val passageCopyCount: Int = 0,
    val personalStyleCount: Int = 0,
    val importedFontCount: Int = 0,
    val signatureReferenceCount: Int = 0,
    val hasTeacherDiagnostic: Boolean = false,
    val hasPreferencesSnapshot: Boolean = false
)

/**
 * Resumo gerado após uma exportação bem-sucedida de backup.
 */
data class BackupSummary(
    val manifest: BackupManifest,
    val totalBytes: Long,
    val fileCount: Int,
    val exportTimestampMs: Long = System.currentTimeMillis()
)

/**
 * Prévia somente-leitura de um pacote antes da restauração.
 */
data class BackupInspectionResult(
    val isValid: Boolean,
    val manifest: BackupManifest? = null,
    val entryCount: Int = 0,
    val totalUncompressedBytes: Long = 0L,
    val errorMessage: String? = null
)

/**
 * Resultado da importação e restauração de um pacote .scribepack.
 */
data class BackupImportResult(
    val isSuccess: Boolean,
    val manifest: BackupManifest? = null,
    val restoredNotebooks: Int = 0,
    val restoredGlyphs: Int = 0,
    val restoredLessons: Int = 0,
    val restoredAttempts: Int = 0,
    val restoredTeacherData: Boolean = false,
    val restoredPassageCopies: Int = 0,
    val restoredStyles: Int = 0,
    val restoredFonts: Int = 0,
    val restoredSignatureReferences: Int = 0,
    val recoveryWasRequired: Boolean = false,
    val errorMessage: String? = null
)
